...
const PARTNER_TYPES = [
  { value: 'wetlands', label: 'Wetlands' }, { value: 'agriculture', label: 'Agriculture' },
  { value: 'urban_ecology', label: 'Urban Ecology' }, { value: 'climate_justice', label: 'Climate Justice' },
...
export function PartnerForm({ partner, onClose, onSaved }: Props) {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      if (partner) { await invoke('partner:update', { id: partner.id, updates: data }); toast.success('Partner updated') }
      else { await invoke('partner:create', data); toast.success('Partner created') }
      onSaved()
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-2xl flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between p-5 border-b border-app-border">
          <h2 className="text-sm font-semibold text-gray-900">{partner ? 'Edit Partner' : 'Add New Partner'}</h2>
          <div className="flex items-center gap-2">
            {hasClaudeKey && !partner && (
              <button type="button" onClick={() => setShowBrochure(!showBrochure)} className="flex items-center gap-1.5 px-2.5 py-1.5 bg-purple-50 text-purple-700 text-xs font-medium rounded-lg hover:bg-purple-100 transition-colors">
                <Sparkles className="w-3.5 h-3.5" />Import from Brochure
              </button>
            )}
        ...
        <div className="overflow-y-auto scrollbar-thin flex-1">
          {showBrochure && (
            <div className="p-4 border-b border-app-border bg-purple-50">
              <p className="text-xs font-semibold text-purple-700 mb-2">Paste partner brochure or description text</p>
              <textarea value={brochureText} onChange={(e) => setBrochureText(e.target.value)} rows={4} className="w-full text-xs border border-purple-200 rounded-lg p-2 bg-white focus:outline-none focus:ring-1 focus:ring-purple-400" placeholder="Paste program description, brochure text, or website content..." />
              <button type="button" onClick={handleExtractBrochure} disabled={extracting || !brochureText.trim()} className="mt-2 flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 text-white text-xs font-medium rounded-lg hover:bg-purple-700 disabled:opacity-50 transition-colors">
                {extracting ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}Extract Fields
              </button>
            </div>
          )}
        ...
        <div className="flex justify-end gap-2 p-4 border-t border-app-border">
          ...
          <button type="submit" form="partner-form" disabled={isSubmitting} className="flex items-center gap-1.5 px-4 py-2 bg-brand text-white text-xs font-medium rounded-lg hover:bg-brand-dark disabled:opacity-50 transition-colors">
            {isSubmitting && <Loader2 className="w-3.5 h-3.5 animate-spin" />}{partner ? 'Save Changes' : 'Create Partner'}
          </button>
        </div>
      </div>
    </div>
