...
export function TopBar({ title, children }: TopBarProps) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 bg-white border-b border-app-border shrink-0">
      <button onClick={toggleSidebar} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 transition-colors" title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}>
      ...
      <div className="flex items-center gap-2">
        ...
        {hasClaudeKey && (
          <button onClick={toggleCopilot} className="flex items-center gap-1.5 px-2 py-1 bg-purple-100 rounded-full text-xs text-purple-700 font-medium hover:bg-purple-200 transition-colors" title="Open AI Copilot">
