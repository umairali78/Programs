...
export function SettingsPage() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [allSettings, teacherList] = await Promise.all([
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Update active teacher in store
        ...
        <div className="max-w-xl space-y-6">
          ...
          <Section title="Active Teacher" description="The teacher whose profile is used for personalized recommendations.">
            <Field label="Select Active Teacher">
              <select
                value={settings.active_teacher_id ?? ''}
                onChange={(e) => updateSetting('active_teacher_id', e.target.value)}
          ...
          <Section title="Map Defaults">
            <Field label="Default Search Radius (miles)">
              <select
                value={settings.default_radius_miles ?? '20'}
                onChange={(e) => updateSetting('default_radius_miles', e.target.value)}
                className={inputClass}
              >
                {[5, 10, 20, 30, 50].map((r) => (
                  <option key={r} value={String(r)}>{r} miles</option>
                ))}
              </select>
            </Field>
          </Section>
          ...
          {/* AI / Claude */}
          <Section title="AI — Claude API" description="Required for matching explanations, AI copilot, digest generation, and standards suggestions.">
          ...
          <Section title="Policy Update" description="AB 2158 status message shown in teacher digests.">
            <Field label="AB 2158 Status Note">
              <textarea
                value={settings.ab2158_status ?? ''}
                onChange={(e) => updateSetting('ab2158_status', e.target.value)}
                rows={3}
                className={inputClass}
              />
            </Field>
