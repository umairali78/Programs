...
export function ProgramForm({ program, partners, onClose, onSaved }: Props) {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      const payload = { ...data, standards }
      if (program) {
        await invoke('program:update', { id: program.id, updates: payload })
        toast.success('Program updated')
      } else {
        await invoke('program:create', payload)
        ...
        <div className="overflow-y-auto scrollbar-thin flex-1">
          <form id="program-form" onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
            <Field label="Partner *" error={errors.partnerId?.message}>
              <select {...register('partnerId')} className={inputClass}>
                {partners.map((p) => (
            ...
            <Field label="Program Title *" error={errors.title?.message}>
              <input {...register('title')} className={inputClass} placeholder="Watershed Explorers" />
            </Field>

            <Field label="Description">
              <textarea {...register('description')} rows={3} className={inputClass} placeholder="What will students experience and learn?" />
            </Field>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {SUBJECTS.map((s) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          const next = field.value.includes(s)
                      ...
                      <button
                        ...
                        className={`px-2 py-0.5 rounded-full text-xs border transition-colors ${
                          field.value.includes(s)
                            ...
                            : 'bg-white text-gray-600 border-gray-200 hover:border-sky'
                        }`}
                      >
                        {s}
                      </button>
            ...
            <div className="grid grid-cols-3 gap-3">
              <Field label="Cost ($)">
                <input {...register('cost', { valueAsNumber: true })} type="number" min="0" step="0.50" className={inputClass} placeholder="0" />
              </Field>
              <Field label="Max Students">
                <input {...register('maxStudents', { valueAsNumber: true })} type="number" min="1" className={inputClass} placeholder="30" />
              </Field>
              <Field label="Duration (min)">
                <input {...register('durationMins', { valueAsNumber: true })} type="number" min="30" className={inputClass} placeholder="120" />
              </Field>
            </div>
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                {hasClaudeKey && program && (
                  <button
                    ...
                    disabled={suggestingStandards}
                    className="flex items-center gap-1 text-xs text-purple-600 hover:text-purple-800"
                  >
                    {suggestingStandards ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                    Suggest Standards
                  </button>
                )}
              </div>
                ...
                <div className="space-y-1 mb-2">
                  {standards.map((s) => (
                    <div key={s.code} className="flex items-center gap-2 text-xs">
                      ...
                      <button
                        type="button"
                        onClick={() => setStandards((p) => p.filter((x) => x.code !== s.code))}
                ...
                <div className="border border-purple-200 rounded-lg p-2 space-y-1 bg-purple-50">
                  ...
                  {suggestedStandards.map((s) => (
                    <button
                      key={s.code}
                      type="button"
                      onClick={() => addSuggestedStandard(s)}
                      className="flex items-center gap-2 w-full text-left text-xs hover:bg-purple-100 rounded px-1 py-0.5 transition-colors"
                    >
